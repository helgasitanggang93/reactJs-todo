...
describe("Todo Details Testing", () => {
  ...
  it("shows title properly", () => {
    expect(wrapped.find("#title-detail-todo").props("title").children).toBe(
      "todo test"
    );
  });
  ...
  it("shows image properly", () => {
    expect(
      wrapped.find("#image-detail-todo").props("image").children.props.src
    ).toBe(
      "http://res.cloudinary.com/dpnjbs730/image/upload/v1574652564/ryglzhwl8..."
    );
    expect(
      wrapped.find("#image-detail-todo").props("image").children.props.style
        .maxWidth
    ).toBe("300px");
    expect(
      wrapped.find("#image-detail-todo").props("image").children.props.style
  ...
  it("shows due_date properly", () => {
    expect(
      wrapped.find("#duedate-detail-todo").props("due_date").children[1]
    ).toBe("Wed, 27 November 2019");
  });

  it("shows description properly", () => {
    expect(
      wrapped.find("#description-detail-todo").props("description").children
    ).toBe("todo test with jest");
  });

  it("show button done properly", () => {
    expect(
      wrapped.find("#buttondone-detail-todo").props("Done").children.props.type
    ).toBe("button");
  });

  it("show button delete properly", () => {
    expect(
      wrapped.find("#buttondelete-detail-todo").props("delete").children.props
        .type
    ).toBe("button");
  });
